package com.okay.appplatform.controller.performance;
...
public class PerformancePadController {
    ...
    @ResponseBody
    public PerformanceHistory insertPerformance(@Validated PerformanceHistory performanceHistory) {
        //插入数据
        ...
        if (FAIL.equals(performanceHistory.getResult()) && ELECTRIC.equals(performanceHistory.getType())) {
            for (PerformanceHistory performanceHistory1 : performanceHistoryList) {
                performanceHistory1.setResult("FAIL");
                reportService.updateReport(performanceHistory1);
            }
        }
