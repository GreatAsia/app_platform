...
import com.okay.appplatform.domain.Page;
import com.okay.appplatform.domain.RetResponse;
import com.okay.appplatform.domain.RetResult;
import com.okay.appplatform.domain.report.UiApkVersion;
...
import com.okay.appplatform.service.ui.*;
...
public class UiPadReportController {
    ...
    public String uiReport(@PathVariable("id") Integer id, Model model) {
        ...
        if ("app".equals(uiPadRunIdList.getRunType())) {
            totalCase = padAutoCaseService.findCaseList().size();

        } else {
            totalCase = romAutoCaseService.findCaseList().size();
        }
        ...
        caseResult.setCountCase(totalCase);
        ...
        List<UiApkVersion> uiApkVersionList = new ArrayList<>();
        ...
        for (UiPadSerialnoList uiPadSerialnoList : uiPadSerialnoLists) {
            if (i == 0) {
                ...
                String[] apkVersions = uiPadSerialnoList.getApkVersion().split(";");
                if (apkVersions.length > 1) {

                    for (String info : apkVersions) {

                        String[] apk = info.split(":");
                        if (apk.length > 1) {
                            UiApkVersion uiApkVersion = new UiApkVersion();
                            uiApkVersion.setAppName(apk[0]);
                            uiApkVersion.setVersion(apk[1]);
                            uiApkVersionList.add(uiApkVersion);
                        } else {
                    ...
                    logger.error("ApkVersion== {}", uiPadSerialnoList.getApkVersion());
                }
                ...
                if (apkVersions.length > 1) {
                    ...
                    for (String info : apkVersions) {
                        ...
                        if (apk.length > 1) {
                            ...
                            for (UiApkVersion u : uiApkVersionList) {
                                if (apk[0].equals(u.getAppName()) && !apk[1].equals(u.getVersion())) {
                                    u.setVersion(u.getVersion() + "/" + apk[1]);
                                    break;
                                }
        ...
        model.addAttribute("uiApkVersionList", uiApkVersionList);
